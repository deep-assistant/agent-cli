...
if (!cloudCodeResponse.ok) {
  const errorBody = await cloudCodeResponse.clone().text().catch(() => 'unknown');

  log.error(() => ({
    message: 'cloud code api error',
    status: cloudCodeResponse.status,
    hint: cloudCodeResponse.status === 500
...
async function checkCloudCodeHealth(): Promise<boolean> {
  try {
    const response = await fetch('https://cloudcode-pa.googleapis.com/v1internal:healthCheck', {
