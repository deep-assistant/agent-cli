...
async function runAgentCli(input, args = []) {
  return new Promise((resolve, reject) => {
    const proc = spawn('bun', ['run', join(process.cwd(), 'src/index.js'), ...args], {
...
function checkForServerStart(stderr) {
...
test('Both modes produce equivalent output structure', async () => {
  ...
  console.log(`Server mode events: ${serverEvents.length}, No-server mode events: ${noServerEvents.length}`)
...
test('Both modes work with custom model parameter', async () => {
  ...
  const serverResult = await runAgentCli(input, ['--server=true', '--model', 'opencode/grok-code'])
  const noServerResult = await runAgentCli(input, ['--no-server', '--model', 'opencode/grok-code'])
...
test('Verify code execution path: server mode calls Server.listen', async () => {
  ...
  const directModeFunctionEnd = indexCode.indexOf('// Explicitly exit', serverModeEnd)
  const directModeFunctionCode = indexCode.substring(serverModeEnd, directModeFunctionEnd)
  ...
  console.log('  - No-server mode uses Session.createNext() and SessionPrompt.prompt()')
})
...
test('Verify no HTTP fetch calls in no-server mode execution path', async () => {
  ...
  const directModeEnd = indexCode.indexOf('// Explicitly exit', directModeStart)
