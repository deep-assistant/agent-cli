...
describe('Claude JSON Standard (experimental)', () => {
  ...
  test('--json-standard claude produces NDJSON format', async () => {
    ...
    const result = await sh(`echo '${input}' | bun run ${projectRoot}/src/index.js --json-standard claude`)
  ...
  test('claude format uses claude event types', async () => {
    ...
    console.log(`✅ Claude format uses valid event types: ${[...new Set(events.map(e => e.type))].join(', ')}`)
  })
  ...
  test('claude format has init event at start', async () => {
    ...
    console.log(`✅ Claude format starts with init event (session_id: ${initEvent.session_id})`)
  })
  ...
  test('claude format has result event at end', async () => {
    ...
    console.log(`✅ Claude format ends with result event (status: ${resultEvent.status})`)
  })
  ...
  test('claude format is different from opencode format', async () => {
    ...
    // Get opencode output
    const opencodeResult = await sh(`echo '${input}' | bun run ${projectRoot}/src/index.js --json-standard opencode`)
    const opencodeStdout = opencodeResult.stdout

    // Get claude output
    const claudeResult = await sh(`echo '${input}' | bun run ${projectRoot}/src/index.js --json-standard claude`)
  ...
  test('claude format event sequence: init -> message(s) -> result', async () => {
    ...
    console.log(`✅ Event sequence is correct: init(${initIndex}) -> message(${messageIndex}) -> result(${resultIndex})`)
  })
})
