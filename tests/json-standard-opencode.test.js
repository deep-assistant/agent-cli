...
describe('OpenCode JSON Standard', () => {
  ...
  test('default output format is opencode (pretty-printed JSON)', async () => {
    ...
    const result = await sh(`echo '${input}' | bun run ${projectRoot}/src/index.js`)
  ...
  test('explicit --json-standard opencode produces same format as default', async () => {
    ...
    // Get default output
    const defaultResult = await sh(`echo '${input}' | bun run ${projectRoot}/src/index.js`)
  ...
  test('opencode format includes required fields', async () => {
    const input = '{"message":"hi"}'
    const result = await sh(`echo '${input}' | bun run ${projectRoot}/src/index.js --json-standard opencode`)
    const events = parseJSONOutput(result.stdout)
  ...
  test('opencode format event types', async () => {
    ...
    const validTypes = ['step_start', 'step_finish', 'text', 'tool_use', 'error']
  ...
  test('opencode format text event has text content', async () => {
    ...
    console.log(`✅ Text event contains response: "${textEvent.part.text.substring(0, 50)}..."`)
  })
  ...
  test('opencode format timestamp is valid Unix milliseconds', async () => {
    ...
    console.log(`✅ All timestamps are valid (between ${beforeTime} and ${afterTime})`)
  })
